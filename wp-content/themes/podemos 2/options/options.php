...
class NHP_Options{
	...
	*/
	function __construct($sections = array(), $args = array(), $extra_tabs = array()){
		...
		$defaults['menu_title'] = __('Opciones', 'mythemeshop');
		$defaults['page_icon'] = 'icon-themes';
		$defaults['page_title'] = __('Opciones', 'mythemeshop');
	...
	function _options_page_html(){
		...
		echo '<div class="wrap">';
			...
			echo '<form method="post" action="options.php" enctype="multipart/form-data" id="nhp-opts-form-wrapper">';
				...
				echo '<a href="www.podemos.info" id="optionpanellogo" class="logo" target="_blank"><img src="'.$this->url.'img/optionpanellogo.png" /></a>';
echo '<span class="headtext"><strong>Options Panel FrameWork, editado para PODEMOS</strong></span>';
				...
				echo '</div>';
					...
					if(isset($_GET['settings-updated']) && $_GET['settings-updated'] == 'true' && get_transient('nhp-opts-saved') == '1'){
						if(isset($this->options['imported']) && $this->options['imported'] == 1){
							echo '<div id="nhp-opts-imported">'.__('<strong>¡Configuración importada!</strong>', 'mythemeshop').'</div>';
						}else{
							echo '<div id="nhp-opts-save">'.__('<strong>¡Configuración guardada!</strong>', 'mythemeshop').'</div>';
						}
						delete_transient('nhp-opts-saved');
					}
					echo '<div id="nhp-opts-save-warn">'.__('<strong>Configuración cambiada, tienes que guardarla</strong>', 'mythemeshop').'</div>';
					echo '<div id="nhp-opts-field-errors">'.__('<strong><span></span> error(s) encontrado</strong>', 'mythemeshop').'</div>';
					...
					echo '<ul id="nhp-opts-group-menu">';
						...
						if(true === $this->args['show_import_export']){
							echo '<li id="import_export_default_section_group_li" class="nhp-opts-group-tab-link-li">';
									echo '<a href="javascript:void(0);" id="import_export_default_section_group_li_a" class="nhp-opts-group-tab-link-a" data-rel="import_export_default"> '.__('Importar / Exportar', 'mythemeshop').'</a>';
							echo '</li>';
				...
				echo '<div id="nhp-opts-main">';
					...
					if(true === $this->args['show_import_export']){
						echo '<div id="import_export_default_section_group'.'" class="nhp-opts-group-tab">';
							echo '<h3>'.__('Importar / Exportar opciones', 'mythemeshop').'</h3>';
							
							echo '<h4>'.__('Importar opciones', 'mythemeshop').'</h4>';
							
							echo '<p><a href="javascript:void(0);" id="nhp-opts-import-code-button" class="button-secondary">Importar código</a></p>';
							...
							echo '<p id="nhp-opts-import-action"><input type="submit" id="nhp-opts-import" name="'.$this->args['opt_name'].'[import]" class="button-primary" value="'.__('Importar', 'mythemeshop').'"> <span>'.apply_filters('nhp-opts-import-warning', __('ADVERTENCIA! La importación de opciones sobrescribirá las opciones existentes, proceda con precaución y haga una exportación de sus opciones previas como copia de seguridad.', 'mythemeshop')).'</span></p>';
							echo '<div id="import_divide"></div>';
							
							echo '<h4>'.__('Exportar opciones', 'mythemeshop').'</h4>';
							echo '<div class="nhp-opts-section-desc">';
								echo '<p class="description">'.apply_filters('nhp-opts-backup-description', __('Aquí, puede exportar y descargar sus parámetros de opciones actuales. Mantenga este seguro, ya que se puede utilizar para copia de seguridad en caso de una emergencia. También puede utilizarlo para restaurar la configuración en este sitio o en cualquier otro sitio que utiliza este tema. También puede copiar el enlace para su configuración, y duplicarlo en otro sitio, lo cual es útil si tiene una red de blogs que todos necesitan la misma configuración.', 'mythemeshop')).'</p>';
							echo '</div>';
							
								echo '<p><a href="javascript:void(0);" id="nhp-opts-export-code-copy" class="button-secondary">Copiar</a></p>';
				...
				echo '<div id="nhp-opts-footer">';
					...
					echo '<input type="submit" name="'.$this->args['opt_name'].'[defaults]" value="'.__('Volver a la configuración previa', 'mythemeshop').'" class="button-secondary" />';
					echo '<input type="submit" name="submit" value="'.__('Guardar cambios', 'mythemeshop').'" class="button-primary" />';
					echo '<div class="clear"></div><!--clearfix-->';
				echo '</div>';
