...
class NHP_Options_color_gradient extends NHP_Options{	
	...
	*/
	function render(){
		...
		echo __('From:', 'nhp-opts').' <input type="text" id="'.$this->field['id'].'-de" name="'.$this->args['opt_name'].'['.$this->field['id'].'][from]" value="'.$this->value['from'].'" class="'.$class.' popup-colorpicker" style="width:70px;"/>';
		echo '<div class="farb-popup"><div class="farb-popup-inside"><div id="'.$this->field['id'].'-frompicker" class="color-picker"></div></div></div>';
		
		echo __(' To:', 'nhp-opts').' <input type="text" id="'.$this->field['id'].'-a" name="'.$this->args['opt_name'].'['.$this->field['id'].'][to]" value="'.$this->value['to'].'" class="'.$class.' popup-colorpicker" style="width:70px;"/>';
