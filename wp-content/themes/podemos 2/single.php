				...
				<div id="post-<?php the_ID(); ?>" <?php post_class('post'); ?>>
					<div class="single_post">
						...
						<div class="post-single-content box mark-links">
							...
							<?php } ?>
							<?php the_content(); ?>
							<?php wp_link_pages(array('before' => '<div class="pagination">', 'after' => '</div>', 'link_before'  => '<span class="current"><span class="currenttext">', 'link_after' => '</span></span>', 'next_or_number' => 'next_and_number', 'nextpagelink' => __('Siguiente','mythemeshop'), 'previouspagelink' => __('Anterior','mythemeshop'), 'pagelink' => '%','echo' => 1 )); ?>
							...
							<?php if($mts_options['mts_tags'] == '1') { ?>
								<!-- Start Tags -->
								<div class="tags"><?php the_tags('<span class="tagtext">'.__('Etiquetas','mythemeshop').':</span>',', ') ?></div>
								<!-- End Tags -->
							<?php } ?>
						</div>
						<!-- End Content -->
						<?php if($mts_options['mts_related_posts'] == '1') { ?>	
							...
							$my_query = new wp_query( $args ); if( $my_query->have_posts() ) {
								echo '<div class="related-posts"><h3>'.__('Entradas relacionadas','mythemeshop').'</h3><div class="postauthor-top"><ul>';
						...
						<?php if($mts_options['mts_author_box'] == '1') { ?>
							...
							<div class="postauthor-container">
								<h4><?php _e('Sobre el autor', 'mythemeshop'); ?></h4>
