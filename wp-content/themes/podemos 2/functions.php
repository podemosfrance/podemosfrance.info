...
function mts_enqueue_css() {
	...
	$custom_css = "
		...
		a:hover, .menu .current-menu-item > a, .menu .current-menu-item, .current-menu-ancestor > a.sf-with-ul, .current-menu-ancestor, footer .textwidget a, .single_post a, #commentform a, .copyrights a:hover, a, footer .widget li a:hover, .menu > li:hover > a, .single_post .post-info a, .post-info a, .readMore a, .reply a, .fn a, .carousel a:hover, .single_post .related-posts a:hover, .sidebar.c-4-12 .textwidget a, footer .textwidget a, .sidebar.c-4-12 a:hover, #logo a, .post-content em, .post-single-content em {color:{$mts_options['mts_color_scheme']}; }	
		.nav-previous a, .nav-next a, .header-button, .sub-menu, #commentform input#submit, .tagcloud a, #tabber ul.tabs li a.selected, .featured-cat, .mts-subscribe input[type='submit'], .pagination a, .post-content h4, .post-content h6, .post-single-content h4, .post-single-content h6 {background-color:{$mts_options['mts_color_scheme']}; color: #fff; }
		.widget li a:hover {color:{$mts_options['mts_color2_scheme']}; }
		.post-content h5, .post-single-content h5, .post-content strong, .post-single-content strong {background-color:{$mts_options['mts_color2_scheme']}; }
		.widget h3 {background-color:{$mts_options['mts_color2_scheme']}; color: #fff; }
		{$mts_sclayout}
...
function mts_widgets_init() {
	register_sidebar(array(
		'name'=>'Sidebar',
		'description'   => __( 'Aparece en los mensajes y las páginas', 'mythemeshop' ),
...
function mts_wp_title( $title, $sep ) {
	...
	if ( $paged >= 2 || $page >= 2 )
		$title = "$title $sep " . sprintf( __( 'Página %s', 'mythemeshop' ), max( $paged, $page ) );
		...
		<div id="comment-<?php comment_ID(); ?>" style="position:relative;">
			<div class="comment-author vcard">
				...
				<span class="comment-meta">
					<?php edit_comment_link(__('(Edtar)', 'mythemeshop'),'  ','') ?>
				</span>
				<span class="reply">
			...
			<?php if ($comment->comment_approved == '0') : ?>
				<em><?php _e('Tu comentario está pendiente de moderación.', 'mythemeshop') ?></em>
				<br />
			<?php endif; ?>
...
echo '<link rel="icon" href="'.site_url().'/wp-content/themes/podemos/images/favicon-admin.png" type="image/x-icon" />';
}
