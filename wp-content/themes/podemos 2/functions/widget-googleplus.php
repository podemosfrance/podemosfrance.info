...
/*-----------------------------------------------------------------------------------
	...
	Plugin Name: PODEMOS GOOGLE+
	Description: Un widget que integra Google+.
	Version: BETA
...
class mts_googleplus_Widget extends WP_Widget {
	...
	{
		$widget_ops = array('classname' => 'mts_googleplus', 'description' => __('Äñadir widget Google+.','mythemeshop'));

		$control_ops = array('id_base' => 'google-badge-box');

		$this->WP_Widget('google-badge-box', __('PODEMOS GOOGLE+ Un widget que integra Google+ en la web.','mythemeshop'), $widget_ops, $control_ops);
	}
...
<script type="text/javascript">
  (function() {
	...
	{
		$defaults = array('title' => __('Encuéntranos en Google Plus','mythemeshop'), 'page_url' => '', 'width' => '300', 'color_scheme' => 'light', 'gp_layout' => 'portrait', 'page_type' => 'profile', 'cover_photo' => 'on', 'tagline' => 'on');
		$instance = wp_parse_args((array) $instance, $defaults); ?>
		
		<p>
			<label for="<?php echo $this->get_field_id('page_type'); ?>"><?php _e('Tipo de página','mythemeshop'); ?>:</label> 
		...
		<p>
			<label for="<?php echo $this->get_field_id('page_url'); ?>"><?php _e('Google+ página URL','mythemeshop'); ?>:</label>
		...
		<p>
			<b><?php _e('Configuración de diseño','mythemeshop'); ?></b>
		</p>
