...
/*-----------------------------------------------------------------------------------*/
	...
	function mts_Subscribe_Widget() {
		...
		/* Create the widget. */
		$this->WP_Widget( 'mts_subscribe_widget', __('PODEMOS POINT WL: Subscribe Widget', 'mythemeshop'), $widget_ops, $control_ops );
	}
		...
		?>
			...
			<div class="mts-subscribe">
                ...
                <form action="http://feedburner.google.com/fb/a/mailverify" method="post" target="popupwindow" onsubmit="window.open('http://feedburner.google.com/fb/a/mailverify?uri=<?php echo $desc; ?>', 'popupwindow', 'scrollbars=yes,width=550,height=520');return true" _lpchecked="1">
					<input type="text" value="<?php _e('Email','mythemeshop'); ?>..." onblur="if (this.value == '') {this.value = '<?php _e('Your email Address','mythemeshop'); ?>...';}" onfocus="if (this.value == '<?php _e('Your email Address','mythemeshop'); ?>...') {this.value = '';}"  name="email">
					<input type="hidden" value="<?php echo $desc; ?>" name="uri"><input type="hidden" name="loc" value="en_US"><input type="submit" value="<?php _e('Suscribir','mythemeshop'); ?>">
				</form>
